...
def main():
    ...
    word_list_inp = load_dict.lc_strings("2of4brif.txt")
    trigrams_filtered = load_dict.lc_strings("least_likely_trigrams.txt")

    word_list = prep_words(name, word_list_inp)
...
def prep_words(name, word_list_inp):
    """Prep word list for finding anagrams"""
    print("length of initial word_list = {}".format(len(word_list_inp)))
    len_name = len(name)
    word_list = [word.lower() for word in word_list_inp if len(word) == len_name]
...
def cv_map_filter(name, filtered_cv_map):
    ...
    print("length of initial permuations set = {}".format(len(perms)))
    vowels = "aeiouy"
    filter_1 = set()
...
def trigram_filter(filter_1, trigrams_filtered):
    """Remove unlikely trigrams from perms"""
    filtered = set()
...
def view_by_letter(name, filter_3):
    ...
    first = input("Select the starting letter or press enter to see all: ")
    subset = []
    for candidate in filter_3:
        if candidate.startswith(first):
            subset.append(candidate)
    print(*sorted(subset), sep="\n")
    print("# of choices starting with {} = {}".format(first, len(subset)))
